...
if (!class_exists('Viral_Mag_Welcome')):
    ...
    class Viral_Mag_Welcome {
        ...
        public function admin_notice_content() {
            ...
            if (!$this->is_dismissed('upgrade') && !empty(get_option('viral_mag_first_activation')) && time() > get_option('viral_mag_first_activation') + 30 * DAY_IN_SECONDS) {
                $this->upgrade_notice();
            }
        ...
        private function screen_notice($hook) {
            ...
            $screens = array(
                'widgets' => array(
                    ...
                    'text' => esc_html__('Viral Pro adds 23 magazine widgets and lets you create unlimited widget areas, each with its own sidebar layout.', 'viral-mag')
                ),
                'nav-menus' => array(
                    'hook' => 'admin_notices',
                    'placement' => 'screen-menus',
                    'text' => esc_html__('Viral Pro adds an in-built MegaMenu and an Off Canvas menu, plus 10 menu hover styles and full menu colour control.', 'viral-mag')
                )
            );
            ...
            <div class="viral-mag-notice notice notice-info">
                ...
                <div class="viral-mag-notice-content">
                    ...
                    <a target="_blank" class="button action" href="<?php echo esc_url(viral_mag_upgrade_url($notice['placement'], 'viral-mag-admin-screen')); ?>"><?php echo esc_html__('See what Viral Pro adds', 'viral-mag'); ?></a>
                </div>
            </div>
            <?php
        }
        ...
        public function block_widgets_notice() {
            ...
            $dismiss_url = add_query_arg(array(
                'viral-mag-hide-notice' => 'screen-widgets',
                'viral_mag_notice_nonce' => wp_create_nonce('screen-widgets')
            ...
            wp_add_inline_script('wp-edit-widgets', sprintf(
                    ...
                    } );', wp_json_encode(__('Viral Pro adds 23 magazine widgets and lets you create unlimited widget areas, each with its own sidebar layout.', 'viral-mag')), wp_json_encode(viral_mag_upgrade_url('screen-widgets', 'viral-mag-admin-screen')), wp_json_encode(__('See what Viral Pro adds', 'viral-mag')), wp_json_encode($dismiss_url), wp_json_encode(__('Don\'t show again', 'viral-mag'))
            ));
        }
                ...
                <div class="viral-mag-notice-content">
                    <p>
                        ...
                        printf(
                            ...
                            esc_html__('You have been running this theme for a month now. %1$sViral Pro%2$s adds 50+ magazine blocks, 7 header layouts, 7 blog and 7 single post layouts, and a lot more - $69 one-time, unlimited sites, lifetime updates, no renewal fees.', 'viral-mag'), '<a style="text-decoration:none;font-weight:bold;" target="_blank" href="' . esc_url(viral_mag_upgrade_url('notice-30day', 'viral-mag-admin-notice')) . '">', '</a>'
                        );
                        ?>
                    </p>
                    <a target="_blank" class="button action" href="<?php echo esc_url(viral_mag_upgrade_url('notice-30day-btn', 'viral-mag-admin-notice')); ?>"><span class="dashicons dashicons-cart"></span><?php echo esc_html__('See Free vs Pro', 'viral-mag'); ?></a> &nbsp;
                    <a class="button action" href="<?php echo esc_url(wp_nonce_url(add_query_arg('viral-mag-hide-notice', 'upgrade'), 'upgrade', 'viral_mag_notice_nonce')); ?>"><span class="dashicons dashicons-no"></span><?php echo esc_html__('No thanks', 'viral-mag'); ?></a>
                </div>
            </div>
            <?php
        }
